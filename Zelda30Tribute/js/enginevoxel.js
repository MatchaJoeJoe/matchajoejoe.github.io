// Copyright 2014 Scott Lininger, All Rights Reserved.

/**
 * @fileoverview Establish the main engine object.
 */
...
ace.EngineVoxel = function(divId, opt_settings) {
  ...
  // For my old, crappy PC.
  if (window.localStorage && window.localStorage['thinkPad']) {
    this.width = 428;
    this.height = 300;
  }

  /**
   * The main game canvas. All the pixel action gets rendered in here.
   */
  this.canvas = this.document.createElement(navigator.isCocoonJS ? "screencanvas": "canvas");
  ...
  /* This is handled in game.js.
  if (navigator.isCocoonJS) {
    setInterval(ace.bind(this.onTick, this), 1000/60);
  } else {
...
ace.EngineVoxel.prototype.drawLight = function(lightImg, worldX, worldY, size, alpha, rotation) {
  ...
  //this.lightMapCtx_.globalAlpha = alpha;
	...
	/* TODO(scott): probably can delete.
	if (doWrap) {
		this.lightMapCtx_.drawImage(lightImg, x + ace.LIGHT_MAP_WIDTH - size/2, y - size/2, size, size);
		this.lightMapCtx_.drawImage(lightImg, x - ace.LIGHT_MAP_WIDTH - size/2, y - size/2, size, size);
		this.lightMapCtx_.drawImage(lightImg, x - size/2, y + lightRoomHeight - size/2, size, size);
		this.lightMapCtx_.drawImage(lightImg, x - size/2, y - lightRoomHeight - size/2, size, size);
  }
  */
};
  ...
  if (game.avatar) {
    ...
    if (isInUnderworld) {
      ...
      this.lightMapCtx_.drawImage($('lightmap'), ace.LIGHT_MAP_WIDTH / 4,
    	...
    	// TODO(scott): Reimplement inside all of the actors.
    	//this.drawLight($('light-lantern'), game.avatar.x, game.avatar.y,100, true);
			//if (game.avatar.flyingSword) {
			//	this.drawLight($('light-lantern'), game.avatar.flyingSword.x, game.avatar.flyingSword.y,100);
			//}
...
ace.EngineVoxel.prototype.setupWebGL_ = function() {
  ...
  //var vs = document.getElementById('vshader-pick').textContent;
  //var fs = document.getElementById('fshader-pick').textContent;
  //this.pickProgram = this.makeProgram(gl, vs, fs);
  ...
  /*
  this.registerVoxelSprite('dodongohead1', 'img/sprites4/dodongohead1.png');
  this.registerVoxelSprite('dodongotail1', 'img/sprites4/dodongotail1.png');
  this.registerVoxelSprite('dodongohead1-flipped', 'img/sprites4/dodongohead1-flipped.png');
  this.registerVoxelSprite('dodongotail1-flipped', 'img/sprites4/dodongotail1-flipped.png');
  */
 ...
 */
ace.EngineVoxel.prototype.drawSingleSprite = function(name, offset, rotZ, opt_shells, opt_rotX, opt_rotX2, opt_renderNegativeColor) {
  mat4.identity(this.uOffsetTransform_);
  mat4.translate(this.uOffsetTransform_, this.uOffsetTransform_, offset);
  if (opt_rotX) {
...
ace.EngineVoxel.prototype.drawRoom = function(name, offset) {
  ...
  gl.uniform1f(this.uSpriteIdLoc, this.voxelSpriteIdsByName_[name]);
  gl.drawArrays(gl.TRIANGLES, this.singleVoxelSpriteVertexCount_, this.roomVertexCount_); //this.roomVertexCount_);
...
ace.EngineVoxel.prototype.populateOnionSkin_ = function(sx, sy, sz) {
   ...
   */
  //var positionList = [0, 16, 1, 15, 2, 14, 3, 13, 4, 12, 5, 11, 6, 10, 7, 9, 8];
  ...
  for (var a = 0; a < layerCount * 3; a++) {
    ...
    */
    ...
    //var innerPosition = innerPositionIndex;

    //console.log(a + '=' + direction + '@' + innerPositionIndex + '=' + innerPosition);
