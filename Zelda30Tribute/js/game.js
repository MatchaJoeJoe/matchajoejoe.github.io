...
/**
 * @fileoverview  Provide the Game class.
...
ace.Game = function(divId, opt_settings) {
   ...
   */
  this.engine =  settings['document'] || new ace.EngineVoxel(divId, opt_settings);
  ...
  this.loadSound('triforcecut', 'sounds/triforcecut.mp3');
...
/**
 * Starts the game a goin'.
 */
ace.Game.prototype.start = function() {
...
ace.Game.prototype.onTick = function(timeStamp) {
  ...
  //this.isLogicFrame = (this.tickCount % this.ticksPerLogicFrame) == 0;
  ...
  //console.clear();
  //console.log(elapsed);
  //console.log(this.idealFrameLength);
  ...
  if (this.isScrolling) {
    ...
    this.updateCamera();
    /*if (this.avatar.x > this.canvasWidth) runner.scrollRoom('right');
    if (this.avatar.x < 0) this.scrollRoom('left');
    if (this.avatar.y > this.canvasHeight) runner.scrollRoom('down');
    if (this.avatar.y < 0) this.scrollRoom('up');
    */
  }
